    ...
    <aside className="h-full w-full max-w-2xl overflow-y-auto border-l border-[var(--border-color)] bg-[var(--bg-main)] shadow-2xl">
      ...
      <div className="space-y-5 p-5">
        ...
        {canReview && <section className="theme-card rounded-xl p-5"><h3 className="font-bold">Human decision</h3><p className="mt-1 text-xs text-[var(--text-secondary)]">Review requires a deliberate decision. Existing backend verification rules are used.</p>
          <div className="mt-4 grid grid-cols-3 gap-2"><Decision icon={Check} label="Confirm" onClick={() => setMode("CONFIRMED")} active={mode === "CONFIRMED"} /><Decision icon={Edit3} label="Correct" onClick={() => setMode("CORRECTED")} active={mode === "CORRECTED"} /><Decision icon={X} label="Reject" onClick={() => setMode("REJECTED")} active={mode === "REJECTED"} /></div>
...
function Decision({ icon: Icon, label, onClick, active }) { return <button type="button" onClick={onClick} className={`rounded-lg border p-3 text-xs font-bold ${active ? "border-orange-500 bg-orange-500/10 text-orange-500" : "border-[var(--border-color)]"}`}><Icon className="mx-auto mb-1 h-4 w-4" />{label}</button>; }
